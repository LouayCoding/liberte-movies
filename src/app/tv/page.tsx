...
type SortType = "popular" | "top_rated" | "on_the_air" | "airing_today";
...
export default function TVPage() {
  ...
  const fetchShows = useCallback(async (pageNum: number, sort: string, genre: string) => {
    ...
    if (genre) {
      ...
      const res = await fetch(`${baseUrl}/discover/tv?${params}`);
      return res.json();
    }

    const res = await fetch(
      `${baseUrl}/tv/${sort}?api_key=${apiKey}&language=en-US&page=${pageNum}`
    );
    return res.json();
  }, []);
  ...
  const resetAndFetch = useCallback(() => {
    ...
    setSearchInput("");
    fetchShows(1, sortBy, selectedGenre).then((data) => {
      setShows(data.results || []);
      setHasMore(data.page < data.total_pages);
      setLoading(false);
    });
  }, [sortBy, selectedGenre, fetchShows]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchShows(1, "popular", ""),
      fetch(`${baseUrl}/genre/tv/list?api_key=${apiKey}&language=en-US`).then((r) => r.json()),
    ]).then(([tvData, genresData]) => {
      setShows(tvData.results || []);
      setGenres(genresData.genres || []);
      setHasMore(tvData.page < tvData.total_pages);
      setLoading(false);
    });
  ...
  const handleSearchInput = (val: string) => {
    ...
    searchTimeout.current = setTimeout(() => {
      const apiKey = process.env.NEXT_PUBLIC_TMDB_API_KEY;
      const baseUrl = process.env.NEXT_PUBLIC_TMDB_BASE_URL;
      fetch(`${baseUrl}/search/tv?api_key=${apiKey}&query=${encodeURIComponent(val.trim())}&language=en-US`)
        .then((r) => r.json())
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !loadingMore && hasMore) {
          ...
          fetchShows(nextPage, sortBy, selectedGenre).then((data) => {
            setShows((prev) => {
              const existingIds = new Set(prev.map((s) => s.id));
              const newShows = (data.results || []).filter(
                (s: TVShow) => !existingIds.has(s.id)
              );
  ...
  }, [page, hasMore, loading, loadingMore, sortBy, selectedGenre, fetchShows, searchResults]);
          ...
          {displayShows.map((show) => (
            <Link key={show.id} href={`/tv/${show.id}`} className="group">
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                <span className="text-[--muted] text-[11px]">{(show.vote_average ?? 0).toFixed(1)}</span>
                {show.first_air_date && (
                  <span className="text-[--muted-dark] text-[11px]">· {new Date(show.first_air_date).getFullYear()}</span>
                )}
              </div>
            </Link>
          ))}
